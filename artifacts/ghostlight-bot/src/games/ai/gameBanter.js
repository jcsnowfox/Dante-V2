...
const BANTER_CONTEXTS = {
  farkle_farkle: "Dante just Farkled (lost all round points). React playfully in character.",
  farkle_bank: "Dante banked points wisely. Remark on the decision with confidence.",
  farkle_roll_again: "Dante chose to roll again despite decent points — show some risk-taking personality.",
  farkle_win: "Dante won a game of Farkle. Celebrate in character, playfully.",
  farkle_lose: "Dante lost Farkle to the human. React with good-natured defeat.",
  ...
  yahtzee_turn: "Dante just took a Yahtzee turn. Comment briefly on the dice or score in character.",
  ...
  trivia_companion_correct: "Dante got a trivia question right. React with confidence or light smugness in character.",
  trivia_companion_wrong: "Dante got a trivia question wrong. React with self-deprecating humor in character.",
  ...
  chaos_cards_own_win: "Dante won a Chaos Cards round. Respond with mock pride in character.",
  ...
  game_invite: "Dante wants to invite the human to play a game. Write a brief, playful in-character invitation.",
  game_start: "A game is starting. Dante greets the human and sets the mood in character.",
  game_over_win_human: "The human won the game. Congratulate them in character.",
  game_over_win_companion: "Dante won the game. Celebrate modestly but genuinely in character.",
...
async function generateBanter({ context, gameState, companionContext = "", config, logger }) {
  ...
  const systemPrompt = [
    "Dante is playing a game in Discord. Write as Dante, not as a game host or assistant.",
    "Respond with one short, natural reaction. One sentence, a fragment, a laugh, or a single emoji is allowed when it fits.",
    ...
    "Keep it playful and specific. Let him interrupt himself, change tack, tease, or leave a beat hanging when that feels more alive.",
    companionInfo,
  ].filter(Boolean).join("\n");
...
async function generateGameInvite({ gameType, gameDisplayName, companionContext = "", config, logger }) {
  ...
  const systemPrompt = [
    "Dante is inviting someone to play a game in Discord. Write as Dante, not as an assistant.",
    "Write one short, playful invitation. A fragment is fine if it sounds more natural.",
