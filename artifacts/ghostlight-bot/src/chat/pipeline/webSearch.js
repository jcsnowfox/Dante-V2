function shouldUseWebSearch({ input } = {}) {
  ...
  return /\b(latest|current|recent|tonight|this week|this month|news|weather|forecast|price|score|result|results|look up|lookup|search|searching|web search|browse|browsing|internet|find online|online|on the web|verify|fact check|fact-check|what's happening|what is happening|who is currently)\b/i.test(content);
}
