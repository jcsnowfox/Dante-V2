 ...
 *
 * Loads and persists the owner's Feedback & Learning settings from
 * companion_system_settings (system_key = feedback_learning). The engine is
 * "active" ONLY when:
...
const { DEFAULT_CONFIG, mergeWithDefaults } = require("./feedbackConfigSchema");

const SYSTEM_KEY = "feedback_learning";

// Mirror emotionStateService: derive companion id from the persona name.
...
function createFeedbackSettingsService({ store, config, logger }) {
  ...
  async function loadSettings({ force = false } = {}) {
    ...
    try {
      ...
      logger.warn("[feedback-learning:settings:loaded] Failed to load settings; treating as inert.", {
        companionId,
        error: error.message,
      });
      cache = inert;
    ...
    logger.debug?.("[feedback-learning:settings:loaded] Settings loaded.", {
      companionId,
      active,
    });
  ...
  async function saveSettings({ enabled, ownerEditable, config: configOverrides }) {
    if (!store) {
      throw new Error("Feedback & Learning storage is not available.");
    }
