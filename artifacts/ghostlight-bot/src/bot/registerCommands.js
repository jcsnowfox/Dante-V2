...
function assertCommandRegistrationConfig(config) {
  ...
  if (!isDiscordSnowflake(config.discord.clientId)) {
    throw new Error("DISCORD_CLIENT_ID must be the numeric Discord application client ID, not a bot token, public key, or client secret.");
  }

  if (!isDiscordSnowflake(config.discord.guildId)) {
    throw new Error("DISCORD_GUILD_ID must be the numeric Discord server ID.");
  }
}
